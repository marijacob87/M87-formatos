...
with st.form("form_calculadora"):
    ...
    with linha1_col3:
        ...
        inverter_papel = st.checkbox("Inverter medidas", key="inverter_papel")

    linha2_col1, linha2_col2, linha2_col3 = st.columns([1, 1, 0.7])

    with linha2_col1:
        peca_largura_txt = st.text_input("Largura da peça (mm)", )

    with linha2_col2:
        peca_altura_txt = st.text_input("Altura da peça (mm)", )

    with linha2_col3:
        st.markdown("<div style='height:28px'></div>", unsafe_allow_html=True)
        inverter_peca = st.checkbox("Inverter medidas", key="inverter_peca")

    linha3_col1, linha3_col2 = st.columns([1, 1.35])
    ...
    with linha3_col2:
        st.markdown("<div style='height:28px'></div>", unsafe_allow_html=True)
        acrescentar_pinca = st.checkbox("Acrescentar 15mm de pinça", key="acrescentar_pinca")
        st.caption("Pinça sempre na parte de baixo da folha horizontal")
    ...
    if papel_largura is not None and papel_altura is not None:
        if inverter_papel:
            papel_largura, papel_altura = papel_altura, papel_largura

        if acrescentar_pinca:
            papel_largura, papel_altura = max(papel_largura, papel_altura), min(papel_largura, papel_altura)

    if peca_largura is not None and peca_altura is not None:
        if inverter_peca:
            peca_largura, peca_altura = peca_altura, peca_largura
...
def calcular(papel_l, papel_a, peca_l, peca_a, espaco, margem, quantidade=None, acrescentar_pinca=False):
    margem_esquerda = margem
    margem_direita = margem
    margem_superior = margem
    margem_inferior = 15 if acrescentar_pinca else margem

    area_util_l = papel_l - margem_esquerda - margem_direita
    area_util_a = papel_a - margem_superior - margem_inferior
    ...
    inicio_x = margem_esquerda + (area_util_l - largura_ocupada) / 2
    inicio_y = margem_inferior + (area_util_a - altura_ocupada) / 2
...
def desenhar(resultado):
    ...
    ax.add_patch(
        patches.Rectangle(
            (
                resultado["margem_esquerda"],
                resultado["margem_inferior"],
            ),
            papel_largura - resultado["margem_esquerda"] - resultado["margem_direita"],
            papel_altura - resultado["margem_superior"] - resultado["margem_inferior"],
            fill=False,
            linestyle="--",
            linewidth=0.7,
            edgecolor="0.65",
...
if melhor or segunda:
    if not campos_principais_ok:
        ...
        if acrescentar_pinca:
            st.write("Pinça: **15 mm na parte inferior da folha horizontal**")